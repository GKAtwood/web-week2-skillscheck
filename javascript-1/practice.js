...
// Check to see if 'x' is equal to true. If it is, return a true boolean.
...
// Have the function take the two parameters and return a 
// string that says "NAMEPARAM loves LOVEPARAM" with the appropriate parameters in the string. e.g. "Joseph loves music"
...
// Create a new array called 'middleNums' from the numbers array that will capture only 
// the middle numbers (2, 3, 4).
...
//  greeter should be a method that returns the string 'Hello! My name is NAMEVALUE and I live in STATEVALUE' 
...
//   Then, loop over the passed in arr parameter, and check to see if the number 
//   in the array is GREATER than 100. If it is, push 'big' as a string to the answers array.
...
let forTheLoveOfGeorge = notGeorge.map(function(currentValue){
	return 'George'
})
...
// // Using the people array, filter out everyone that isn't a friend into a new array called 'enemies'.
//  Use .filter(). 
...
let enemies = people.filter(function(friends){
